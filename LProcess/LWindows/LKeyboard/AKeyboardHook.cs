...
namespace SputnikAsm.LProcess.LWindows.LKeyboard
{
    public class AKeyboardHook : IAApplied
    {
        public delegate void KeyDownEventDelegate(AKeyboardHookEventArgs e);

        public delegate void KeyUpEventDelegate(AKeyboardHookEventArgs e);
        ...
        public AKeyboardHook(string name)
        {
            Identifier = name;
        }
            ...
            try
            {
                if (!IsPaused && code >= 0)
                {
                    if (wParam.ToInt32() == WmSyskeydown || wParam.ToInt32() == WmKeydown)
                        KeyDownEvent(new AKeyboardHookEventArgs(lParam));

                    if (wParam.ToInt32() == WmSyskeyup || wParam.ToInt32() == WmKeyup)
                        KeyUpEvent(new AKeyboardHookEventArgs(lParam));
                }
            }
            finally
            {
